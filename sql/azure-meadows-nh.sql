...
CREATE TABLE IF NOT EXISTS azure_meadows_nh.l_activity_types (
...
CREATE TABLE IF NOT EXISTS azure_meadows_nh.neighbors (
  ...
  DISCORD_SNOWFLAKE STRING NOT NULL,
  NEIGHBOR_ROLE STRING CHECK (NEIGHBOR_ROLE IN ('Member', 'Elder', 'Co-Leader', 'Leader')) DEFAULT 'Member',
  NEIGHBOR_ACTIVE BOOLEAN DEFAULT true
);
...
CREATE TABLE IF NOT EXISTS azure_meadows_nh.l_action_types (
...
CREATE TABLE IF NOT EXISTS azure_meadows_nh.action_log (
  ...
  NEIGHBOR_ID INT8 REFERENCES azure_meadows_nh.neighbors(NEIGHBOR_ID) NOT NULL,
  ACTION_TYPE_ID INT8 REFERENCES azure_meadows_nh.action_types(ACTION_TYPE_ID) NOT NULL,
  ACTION_LOG_NOTE STRING DEFAULT NULL
);

CREATE TABLE IF NOT EXISTS azure_meadows_nh.l_derby_types (
...
CREATE TABLE IF NOT EXISTS azure_meadows_nh.derby_history (
  DERBY_START_DATE DATE PRIMARY KEY CHECK (date_part('dayofweek', DERBY_START_DATE) = 2) NOT NULL,
  DERBY_TYPE_ID INT8 REFERENCES azure_meadows_nh.l_derby_types (DERBY_TYPE_ID) NOT NULL,
  DERBY_PLACEMENT INT8 NOT NULL
);
...
CREATE TABLE IF NOT EXISTS azure_meadows_nh.derby_participation (
  ...
  NEIGHBOR_ID INT8 REFERENCES azure_meadows_nh.neighbors(NEIGHBOR_ID) NOT NULL,
  TASKS_TAKEN INT8 NOT NULL,
  TASKS_ASSIGNED INT8 NOT NULL,
  DERBY_POINTS INT8 NOT NULL
);


CREATE VIEW azure_meadows_nh.v_derby_standings AS
  SELECT 
      dh.DERBY_START_DATE
    , dt.DERBY_TYPE_NAME
  ...
  INNER JOIN azure_meadows_nh.l_derby_types AS dt 
...
-- General view
CREATE VIEW azure_meadows_nh.v_neighbor_view AS
  SELECT 
      nh.FARM_NAME
    , nh.NEIGHBOR_ROLE
  ...
  FROM azure_meadows_nh.neighbors AS nh
    LEFT JOIN azure_meadows_nh.action_log AS al_join_date
      ON al_join_date.NEIGHBOR_ID = nh.NEIGHBOR_ID 
        AND al_join_date.ACTION_TYPE_ID = (SELECT ACTION_TYPE_ID 
                              FROM azure_meadows_nh.l_action_types 
    ...
    LEFT JOIN (
      ...
      FROM azure_meadows_nh.activity_check_log AS acl
        INNER JOIN azure_meadows_nh.l_activity_types AS at
...
CREATE VIEW azure_meadows_nh.v_derby_details AS
  SELECT 
      dh.derby_start_date
    , ldt.derby_type_name
    , n.farm_name
    ...
    , dp.tasks_assigned
    , dp.derby_points
  FROM azure_meadows_nh.derby_history dh
    INNER JOIN azure_meadows_nh.l_derby_types ldt
      on ldt.derby_type_id = dh.derby_type_id
    INNER JOIN azure_meadows_nh.derby_participation dp 
      ON dp.derby_start_date = dh.derby_start_date
    INNER JOIN azure_meadows_nh.neighbors n
      ON n.neighbor_id = dp.neighbor_id
  ORDER BY 1
  ;